...
class RTP_Session : public PObject
{
  ...
  ///LDLac code
  virtual void mcu_session_set_clock_rate(int tmp){
    clock_rate = tmp;
  }

  virtual int mcu_session_get_clock_rate(){
    return clock_rate;
  }
  virtual float mcu_session_get_current_quality(){
    return QOS_R;
  }
  virtual float mcu_session_set_current_quality(float tmp){
    //static float R = 0.0;
    QOS_R = tmp;
    return QOS_R;
  }
