...
#define _CHECK_INTERVAL_LAST_CHANGE_DECREASE 30
/////////// end LDLac code
#if USE_FREETYPE
...
BOOL MCUPVideoInputDevice::GetFrameData(BYTE * buffer, PINDEX * bytesReturned)
{
  ...
  *///
  ...
  if(code_relay != -1){
    //CLogger::getLogger()->Log("ID %d co QOS la %d", member_now->GetID(), code_relay);
    switch (code_relay) {
   		case 3:
  		  //480p 704x576 , fps =15
        //fps
...
int Get_code(float R_Qos){
	if(R_Qos > 3.5) return 3;
	if(R_Qos > 2.0) return 2;
	if(R_Qos > 0.5) return 1;
                                              ...
                                              BOOL /*endFrame*/)
{
  ...
  if (difftime(NOW,info_count.sum_quality_timer) >= _CHECK_INTERVAL_ ){
    ...
    if (info_count.tb_change_R == 0){
      ...
      CLogger::getLogger()->Log("member id %d co QOS %f, count %d",member->GetID(), mcuConnection.mcu_connection_get_current_quality(), info_count.count_sum);
      ...
      if (code_of_R != pre_code_of_R) {
        ...
        //------Neu network up
        if(code_of_R > pre_code_of_R && info_count.count_increase == 2 ){
          //Tang fps vs res
          //CLogger::getLogger()->Log("tang fps and res");
