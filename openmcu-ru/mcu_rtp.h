...
class MCU_RTPChannel : public H323_RTPChannel
{
  ...
  public:
    ...
    ///LDLac code
    void set_id_for_channel(int id){
      idMember = id;
    }

    int get_id_from_channel(){
      return idMember;
    }

    virtual void mcu_channel_set_clock_rate(int tmp){
      rtpSession.mcu_session_set_clock_rate(tmp);
    }
    virtual int mcu_channel_get_clock_rate(){
      return rtpSession.mcu_session_get_clock_rate();
    }

    virtual float mcu_channel_get_quality(){
      return rtpSession.mcu_session_get_current_quality();
    }
    ///LDLac code end
  protected:
    int idMember;
...
struct QOS_info{
  int last_ext_seq_num;
  int last_packet_sent_count_num;
  int last_cum_loss_num;
  PTimeInterval last_estimate_time_ms;
};
