...
{
  //CLogger::getLogger()->Log("heloo process SDP abc");
...
void SipCapability::Print()
{
 ...
 //CLogger::getLogger()->Log("da print ra roi cha oi");
}
...
void MCUSipConnection::StartMediaChannel(MediaTypes mtype, int pt, int dir)
{
  ...
  if(dir == 0 && (sc->mode&2) && sc->inpChan && !sc->inpChan->IsRunning()){ // nhan video
     sc->inpChan->Start();
     //CLogger::getLogger()->Log("hello 1");
  }
  if(dir == 1 && (sc->mode&1) && sc->outChan && !sc->outChan->IsRunning()){ // truyen di
    sc->outChan->Start();
    sc->outChan->mcu_channel_set_clock_rate(sc->clock);//----LDLac code
    //CLogger::getLogger()->Log("hello 3");
  }
}
...
void MCUSipConnection::StartMediaChannels()
{
  ...
  //CLogger::getLogger()->Log("start channels");
}
...
int MCUSipConnection::ProcessSDP(SipCapMapType & LocalCaps, SipCapMapType & RemoteCaps, PString & sdp_str)
{
  ...
  //CLogger::getLogger()->Log("heloo process SDP audio");
  ...
  if(vcap >= 0)
  {
    ...
    if(sc){
      sc->Print();
      //CLogger::getLogger()->Log("heloo process SDP video");
    }
  }
  return 0;
}
...
int MCUSipConnection::ProcessConnect()
{
  ...
  SetMemberName();
  //CLogger::getLogger()->Log("hello a3");
  // join conference
  ...
  //
  OnEstablished();
  //CLogger::getLogger()->Log("connect xong");
  return 0;
}
...
int MCUSipConnection::ProcessInvite(const msg_t *msg)
{
  ...
  //CLogger::getLogger()->Log("xong invite");
  return 0;
}
...
int MCUSipEndPoint::CreateIncomingConnection(const msg_t *msg)
{
  ...
  //CLogger::getLogger()->Log("hello %d", response_code);
  sCon->Unlock();
  return 0;
}
