...
void ConferencePipeMember::VideoThread(PThread &, INT)
{
  ...
  while (running) {
    // read a block of data
    if(videoMixer!=NULL) videoMixer->ReadFrame(*this,videoData.GetPointer(),width,height,amount,member_info.videoType);//LDLac code
    else conference->ReadMemberVideo(this,videoData.GetPointer(),width,height,amount, member_info.videoType);//LDLac code
